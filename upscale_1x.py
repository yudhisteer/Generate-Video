...
upscale_payload = json.dumps({
    "parent_task_id": "f9ac580f-85fe-4880-9424-b3a5c81ed62a",
...
try:
    ...
    while retry_count < max_retries and status in ["staged", "processing"] and (time.time() - start_time) < timeout:
        time.sleep(min(5 * (1.5 ** retry_count), 60))  # Exponential backoff, max 60 seconds
    ...
    if image_url:
        ...
        # Save URL to a file
        with open('upscaled_1x_url.txt', 'w') as f:
            f.write(f"Upscaled Task ID: {task_id}\n")
